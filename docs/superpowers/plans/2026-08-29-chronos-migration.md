...
#
# 状态：除「全链路 PIE / 完整回归 / 最终回归」三项外，其余步骤均已执行完成并落盘。
# 这三项需在 Rider 构建后于编辑器内实机验证，通过后再勾选关闭。
...
- [x] **Step 1: ChronosWeapon.h 添加表现事件**
...
- [x] **Step 2: ChronosWeapon.cpp 调用钩子 + 允许空投射物类型**
...
- [x] **Step 3: CombatComponent.cpp 修复 EquipTo 缺口**
...
- [x] **Step 4: 移除近战（ChronosCharacter）**
...
- [x] **Step 7: 创建 docs/Architecture.md 骨架**
...
- [x] **Step 1: 写失败的测试**（Tests/ChronosGameFlowTests.cpp）
...
- [x] **Step 5: 实现 ChronosGameMode.cpp**
...
- [x] **Step 9: 更新 docs/Architecture.md 的"C++ 单元"节**（追加 AChronosGameMode/AChronosEnemy 行）+ **Step 10: 提交**
...
- [x] **Step 1: ChronosPlayerController.h**
...
- [x] **Step 2: ChronosPlayerController.cpp**
...
- [x] **Step 3: 构建（.h 变更 → 全量+重启）→ Step 4: 更新 Architecture.md + Step 5: 提交**
...
- [x] **Step 1: describe_toolset BlueprintTools/AssetTools/DataAssetTools/ObjectTools**，确认 `set_parent/compile_blueprint/remove_variable/remove_function_graph`、资产创建/删除/保存、DataAsset 创建与属性写入的入参。

- [x] **Step 2: 重父类 BP_ShooterWeaponBase → AChronosWeapon**
...
- [x] **Step 9: 更新 Architecture.md（蓝图资产/数据资产节）+ Step 10: 提交**（资产 .uasset 与文档一起 `git add` 具体路径）
...
- [x] **Step 1: C++——订阅时间缩放并写入 Niagara 用户参数**
...
- [x] **Step 3: BP_ChronosProjectile + DA 指向**
...
- [x] **Step 4: 全量构建 + 重启（.h 变更）→ Step 5: PIE 验证**

PIE：正常速度弹道暗红；对墙开枪后立即停止输入（时间冻结）观察弹道变亮悬停。异常修复重跑。

- [x] **Step 6: 更新 Architecture.md + Step 7: 提交**
...
- [x] **Step 5: 早期风险验证——时间冻结下 StateTree 是否静止**（规格 §9 风险表要求）
...
- [x] **Step 1: describe UMGToolSet**，按其工作流（list_properties → get/set_properties）创建。
...
- [x] **Step 4: WBP_Victory / WBP_Defeat**

全屏半透明黑底；Victory 大字 `YOU WIN`（绿色）；Defeat 大字 `DEFEAT`（红色）+ 小字 `按 R 重试`。

- [x] **Step 5: 保存 + Step 6: 更新 Architecture.md + Step 7: 提交**
...
- [x] **Step 4: 更新 Architecture.md + Step 5: 提交**
...
- [x] **Step 2: GameMode LevelList 配置**
...
- [x] **Step 4: 修复发现的问题并重跑** → **Step 5: 更新 Architecture.md（关卡节）+ Step 6: 提交**
...
- [x] **Step 6: 修复重跑 → Step 7: 更新 Architecture.md → Step 8: 提交**
...
- [x] **Step 2: Architecture.md 终校**：与实际资产/类清单逐项核对（MCP `list` 类工具抽查），删除过时描述。
